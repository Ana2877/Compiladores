...
assign_array_right: expression RIGHT_ASSIGN array_with_expression    {astPrint($3, 0)};
assign_array_left: array_with_expression LEFT_ASSIGN expression    {astPrint($3, 0)};
...
expression: array_with_expression
            ...
            | LIT_TRUE
            | LIT_FALSE
            | expression '+' expression  { $$ = AST_create(AST_ADD, 0, $1, $3, 0); }
            | expression '-' expression  { $$ = AST_create(AST_SUB, 0, $1, $3, 0); }
            | expression '*' expression  { $$ = AST_create(AST_MULTIPLY, 0, $1, $3, 0); }
            | expression '/' expression  { $$ = AST_create(AST_DIVIDE, 0, $1, $3, 0); }
            | expression '>' expression  { $$ = AST_create(AST_GT, 0, $1, $3, 0); }
            | expression '<' expression  { $$ = AST_create(AST_LT, 0, $1, $3, 0); }
            | expression '|' expression  { $$ = AST_create(AST_OR, 0, $1, $3, 0); }
            | expression '&' expression  { $$ = AST_create(AST_AND, 0, $1, $3, 0); }
